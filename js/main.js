...
// --- CORE LOGIC (RESTRUCTURED FOR PERFORMANCE) ---

/**
 * NEW: This function now runs first to get the UI updated ASAP.
 */
async function initializeFirebaseAndListen() {
  try {
    ...
    // Start listening for settings from Firestore
    ...
    onSnapshot(settingsRef, (doc) => {
      if (doc.exists()) {
        ...
        // If no settings exist, apply a sensible default immediately.
        // It might get updated by the GitHub fetch later, but this prevents a blank state.
        applySettings({
          wallpaperUrl: "https://i.postimg.cc/W1V6yC1g/windows-7-wallpaper.jpg",
...
/**
 * NEW: This function now runs in the background to fetch wallpaper options.
 */
async function populateWallpaperOptions() {
  if (
  ...
  try {
    ...
    dynamicWallpapers = wallpapersData
    ...
    // If this is the very first load and no wallpaper was set from Firebase,
    // update the default to the first one from GitHub.
    if (!currentSettings.wallpaperUrl && dynamicWallpapers.length > 0) {
      applySettings({
        ...currentSettings,
        wallpaperUrl: dynamicWallpapers[0].url,
      });
    }
  } catch (error) {
...
function openPersonalizeModal() {
  ...
  wallpaperOptionsContainer.innerHTML = ""; // Clear previous options
  if (dynamicWallpapers.length === 0) {
    wallpaperOptionsContainer.innerHTML = `<p class="col-span-4 text-center text-sm">Could not load wallpapers from GitHub.</p>`;
    return;
  }
...
function initializeDesktop() {
  // Start both tasks at the same time. Neither waits for the other.
  initializeFirebaseAndListen();
  populateWallpaperOptions();

  // The rest of the UI setup can happen immediately.
  setupUI();
}
...
function setupUI() {
  ...
  document.querySelectorAll(".win7-window").forEach((windowEl) => {
    ...
    controls.querySelector(".minimize-btn")?.addEventListener("click", () => {
      windowEl.classList.add("hidden");
      if (
        !document.querySelector(`.taskbar-btn[data-window-id="${windowId}"]`)
      ) {
...
// --- Draggable Function Definitions (UNCHANGED) ---
